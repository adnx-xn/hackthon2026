...
class ADCPParser(ObservationParser):
    ...
    def get_metadata(self, source: str, source_type: str = 'local') -> ObsDatasetRecord:
        with self._open_dataset(source, source_type) as ds:
            ...
            if 'platform_id' in ds:
                ...
                platforms = [f"ADCP_{i}" for i in range(len(time_vals))]
            ...
            for i in range(len(platforms)):
                ...
                if pid not in instruments:
                    ...
                    # Extract full depth profile of u and v for ADCP vectors
                    u_prof = ds['u'].isel(time=i).values if 'u' in ds else []
                    v_prof = ds['v'].isel(time=i).values if 'v' in ds else []
                    depths = ds['depth'].values if 'depth' in ds else []
                    
                    u_list = [float(x) if not np.isnan(x) else None for x in np.atleast_1d(u_prof)]
                    v_list = [float(x) if not np.isnan(x) else None for x in np.atleast_1d(v_prof)]
                    d_list = [float(x) if not np.isnan(x) else None for x in np.atleast_1d(depths)]
            ...
            return ObsDatasetRecord(
                id=dataset_id,
                name=f"ADCP Dataset {dataset_id}",
                source=source,
    ...
    def get_profile(self, source: str, source_type: str, instrument_id: str) -> ProfileData:
        with self._open_dataset(source, source_type) as ds:
            if 'platform_id' in ds:
                ...
                platforms = [f"ADCP_{i}" for i in range(len(ds['time']))]
